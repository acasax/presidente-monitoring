...
const AttendanceDatePickerModeSelect = () => {
  ...
  const handleChange = (event: SelectChangeEvent<typeof dataPickerMode>) => {
    ...
    dispatch(clearAttendancePickedDate());
    dispatch(setAttendanceDatePickerMode(typeof value === 'string' ? value.split(',') : value));
  };
  ...
  return (
    <CustomSelect
      ...
      id="date-picker-mode-attendance"
    />
  );
};

export default AttendanceDatePickerModeSelect;
